...
public class UIControllerCorgi : MonoBehaviour
{
    ...
    void Update()
    {
        HandlePCANInput(); // Process IMU data first (before UI updates)
        HandleButtonInput();
        ...
        while (isThreadRunning)
        {
            try
            {
                if (serialPort != null && serialPort.IsOpen)
                {
                    ...
                    // Immediately process PCAN input after receiving serial data
                    HandlePCANInput();

                    // Reduce delay between serial reads
                    Thread.Sleep(1); // Small delay to prevent CPU overuse
                }
            }
