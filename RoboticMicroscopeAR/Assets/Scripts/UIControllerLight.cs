...
public class UIControllerLight: MonoBehaviour
{
    ...
    {
        if (lastHoveredUI == hoverLightBulb)
        {
    ...
    private void ExitLightBulbMode()
    {
        ...
        // Start a coroutine to hide the slider and then load the next scene
    ...
    private IEnumerator TransitionToNextScene()
    {
        ...
        if (preLightBulbContainer != null)
        {
            preLightBulbContainer.SetActive(false); // Hide UI
        }

        yield return new WaitForSeconds(1f); // Small delay before transition

        Debug.Log("Loading Next Scene: Intro 3");
        SceneManager.LoadScene("Intro3"); // ✅ Ensure the scene name matches your Unity scene
    }
